...
def main(train_file, model_output, test_file=None, test_output=None):
    ...
    df['is_ai_ml'] = df['themes'].apply(lambda theme: 1 if theme == 'AI or Machine Learning' else 0)
    ...
    if test_file:
        ...
        grid_search.fit(df['postContent'], df['is_ai_ml'])
        ...
        best_model.fit(df['postContent'], df['is_ai_ml'])
        ...
        # Ensure the test file has the 'is_ai_ml' column for evaluation
        if 'themes' in df_test.columns:
            df_test['is_ai_ml'] = df_test['themes'].apply(lambda theme: 1 if theme == 'AI or Machine Learning' else 0)
        else:
            raise KeyError("The test file must contain a 'themes' column to create 'is_ai_ml' labels for evaluation.")

        predictions = best_model.predict(df_test['postContent'])
        df_test['prediction'] = predictions

        print("Classification Report on the external test set:")
        print(classification_report(df_test['is_ai_ml'], predictions))
        print("Accuracy on the external test set:", accuracy_score(df_test['is_ai_ml'], predictions))
        ...
        X_train, X_test, y_train, y_test = train_test_split(df['postContent'], df['is_ai_ml'], test_size=0.2, random_state=42)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a binary classifier for 'AI or Machine Learning' theme.")
