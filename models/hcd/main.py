...
def main(train_file, model_output, test_file=None, test_output=None):
    ...
    df['is_hcd'] = df['themes'].apply(lambda theme: 1 if theme == 'Human-Centered Design' else 0)
    ...
    if test_file:
        ...
        grid_search.fit(df['postContent'], df['is_hcd'])
        ...
        best_model.fit(df['postContent'], df['is_hcd'])
        ...
        # Ensure the test file has the 'is_hcd' column for evaluation
        if 'themes' in df_test.columns:
            df_test['is_hcd'] = df_test['themes'].apply(lambda theme: 1 if theme == 'Human-Centered Design' else 0)
        else:
            raise KeyError("The test file must contain a 'themes' column to create 'is_hcd' labels for evaluation.")

        predictions = best_model.predict(df_test['postContent'])
        df_test['prediction'] = predictions

        print("Classification Report on the external test set:")
        print(classification_report(df_test['is_hcd'], predictions))
        print("Accuracy on the external test set:", accuracy_score(df_test['is_hcd'], predictions))
        ...
        X_train, X_test, y_train, y_test = train_test_split(df['postContent'], df['is_hcd'], test_size=0.2, random_state=42)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a binary classifier for 'Human-Centered Design' theme.")
