...
def main(train_file, model_output, test_file=None, test_output=None):
    ...
    df['is_health_outcomes'] = df['theme'].apply(lambda theme: 1 if theme == 'Health outcomes' else 0)
    ...
    if test_file:
        ...
        grid_search.fit(df['postContent'], df['is_health_outcomes'])
        print(f"Best parameters: {grid_search.best_params_}")
        print(f"Best cross-validation accuracy: {grid_search.best_score_}")
        best_model = grid_search.best_estimator_
        best_model.fit(df['postContent'], df['is_health_outcomes'])
        ...
        if 'theme' in df_test.columns:
            df_test['is_health_outcomes'] = df_test['theme'].apply(lambda theme: 1 if theme == 'Health outcomes' else 0)
        else:
            raise KeyError("The test file must contain a 'theme' column to create 'is_health_outcomes' labels for evaluation.")
        predictions = best_model.predict(df_test['postContent'])
        df_test['prediction'] = predictions
        print("Classification Report on the external test set:")
        print(classification_report(df_test['is_health_outcomes'], predictions))
        print("Accuracy on the external test set:", accuracy_score(df_test['is_health_outcomes'], predictions))
        df_test.to_csv(test_output, index=False)
        print(f"Predictions saved to {test_output}")
    else:
        X_train, X_test, y_train, y_test = train_test_split(df['postContent'], df['is_health_outcomes'], test_size=0.2, random_state=42)
        grid_search = GridSearchCV(pipeline, param_grid, cv=5, scoring='accuracy')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a binary classifier for 'Health outcomes' theme.")
