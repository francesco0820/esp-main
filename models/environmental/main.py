...
def main(train_file, model_output, test_file=None, test_output=None):
    ...
    df['is_environmental_sustainability_climate'] = df['theme'].apply(lambda theme: 1 if theme == 'Environmental Sustainability and Climate' else 0)
    ...
    if test_file:
        ...
        grid_search.fit(df['postContent'], df['is_environmental_sustainability_climate'])
        print(f"Best parameters: {grid_search.best_params_}")
        print(f"Best cross-validation accuracy: {grid_search.best_score_}")
        best_model = grid_search.best_estimator_
        best_model.fit(df['postContent'], df['is_environmental_sustainability_climate'])
        ...
        if 'theme' in df_test.columns:
            df_test['is_environmental_sustainability_climate'] = df_test['theme'].apply(lambda theme: 1 if theme == 'Environmental Sustainability and Climate' else 0)
        else:
            raise KeyError("The test file must contain a 'theme' column to create 'is_environmental_sustainability_climate' labels for evaluation.")
        predictions = best_model.predict(df_test['postContent'])
        df_test['prediction'] = predictions
        print("Classification Report on the external test set:")
        print(classification_report(df_test['is_environmental_sustainability_climate'], predictions))
        print("Accuracy on the external test set:", accuracy_score(df_test['is_environmental_sustainability_climate'], predictions))
        df_test.to_csv(test_output, index=False)
        print(f"Predictions saved to {test_output}")
    else:
        X_train, X_test, y_train, y_test = train_test_split(df['postContent'], df['is_environmental_sustainability_climate'], test_size=0.2, random_state=42)
        grid_search = GridSearchCV(pipeline, param_grid, cv=5, scoring='accuracy')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a binary classifier for 'Environmental Sustainability and Climate' theme.")
