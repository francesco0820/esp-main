...
def main(train_file, model_output, test_file=None, test_output=None):
    ...
    df['is_rd'] = df['themes'].apply(lambda theme: 1 if theme == 'Research and Development' else 0)
    ...
    if test_file:
        ...
        grid_search.fit(df['postContent'], df['is_rd'])
        ...
        best_model.fit(df['postContent'], df['is_rd'])
        ...
        # Ensure the test file has the 'is_rd' column for evaluation
        if 'themes' in df_test.columns:
            df_test['is_rd'] = df_test['themes'].apply(lambda theme: 1 if theme == 'Research and Development' else 0)
        else:
            raise KeyError("The test file must contain a 'themes' column to create 'is_rd' labels for evaluation.")

        predictions = best_model.predict(df_test['postContent'])
        df_test['prediction'] = predictions

        print("Classification Report on the external test set:")
        print(classification_report(df_test['is_rd'], predictions))
        print("Accuracy on the external test set:", accuracy_score(df_test['is_rd'], predictions))
        ...
        X_train, X_test, y_train, y_test = train_test_split(df['postContent'], df['is_rd'], test_size=0.2, random_state=42)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a binary classifier for 'Research and Development' theme.")
