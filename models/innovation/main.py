...
def main(train_file, test_file, model_output, test_output):
    ...
    df['is_innovation'] = df['themes'].apply(lambda theme: 1 if theme == 'Innovation' else 0)

    # Vectorizer to transform the text data
    vectorizer = TfidfVectorizer(stop_words=stop_words, max_features=10000)

    # Split the data
    X_train, X_test, y_train, y_test = train_test_split(df['postContent'], df['is_innovation'], test_size=0.2, random_state=42)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a binary classifier for 'Innovation' theme.")
